...
/datum/shuttle/ferry/emergency/launch(var/user)
	...
	if (istype(user, /obj/machinery/computer/shuttle_control/emergency))	//if we were given a command by an emergency shuttle console
		if (emergency_shuttle.autopilot)
			...
			world << SPAN_NOTE("<b>Alert: The shuttle autopilot has been overridden. Launch sequence initiated!</b>")
...
/datum/shuttle/ferry/emergency/force_launch(var/user)
	...
	if (istype(user, /obj/machinery/computer/shuttle_control/emergency))	//if we were given a command by an emergency shuttle console
		if (emergency_shuttle.autopilot)
			emergency_shuttle.autopilot = 0
			world << SPAN_NOTE("<b>Alert: The shuttle autopilot has been overridden. Bluespace drive engaged!</b>")
...
/datum/shuttle/ferry/emergency/cancel_launch(var/user)
	...
	if (istype(user, /obj/machinery/computer/shuttle_control/emergency))	//if we were given a command by an emergency shuttle console
		if (emergency_shuttle.autopilot)
			emergency_shuttle.autopilot = 0
			world << SPAN_NOTE("<b>Alert: The shuttle autopilot has been overridden. Launch sequence aborted!</b>")
...
/obj/machinery/computer/shuttle_control/emergency/proc/read_authorization(var/obj/item/ident)
	...
	if (req_authorizations - authorized.len)
		world << SPAN_NOTE("<b>Alert: [req_authorizations - authorized.len] authorization\s needed to override the shuttle autopilot.</b>")
...
/obj/machinery/computer/shuttle_control/emergency/emag_act(var/remaining_charges, var/mob/user)
	if (!emagged)
		user << SPAN_NOTE("You short out \the [src]'s authorization protocols.")
		emagged = 1
		return 1
