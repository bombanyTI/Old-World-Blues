...
/obj/machinery/atmospherics/unary/vent_pump/attackby(obj/item/W, mob/user)
	if(istype(W, /obj/item/weapon/weldingtool))
		...
		if (WT.remove_fuel(0,user))
			user << SPAN_NOTE("Now welding the vent.")
			if(do_after(user, 20, src))
				if(!src || !WT.isOn()) return
				playsound(src.loc, 'sound/items/Welder2.ogg', 50, 1)
				if(!welded)
					user.visible_message(
						SPAN_NOTE("\The [user] welds the vent shut."),
						SPAN_NOTE("You weld the vent shut."),
						"You hear welding."
					)
					welded = 1
					update_icon()
					...
					user.visible_message(
						SPAN_NOTE("[user] unwelds the vent."),
						SPAN_NOTE("You unweld the vent."),
						"You hear welding."
					)
					welded = 0
					update_icon()
			else
				user << SPAN_NOTE("The welding tool needs to be on to start this task.")
		else
