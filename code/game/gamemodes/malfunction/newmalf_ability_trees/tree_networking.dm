...
/datum/game_mode/malfunction/verb/basic_encryption_hack(obj/machinery/power/apc/A as obj in get_unhacked_apcs(src))
	...
	if(A)
		if(A.hacker && A.hacker == user)
			...
			user << SPAN_NOTE("Unable to connect to APC. Please verify wire connection and try again.")
			return
	else
		return
	...
	if(A)
		...
		if(A.hacker == user)
			...
			user << SPAN_NOTE("Hack failed. Connection to APC has been lost. Please verify wire connection and try again.")
	else
		user << SPAN_NOTE("Hack failed. Unable to locate APC. Please verify the APC still exists.")
	user.hacking = 0
